...
public class FreemarkerServlet extends HttpServlet {
    ...
    private static final String REDIRECT_URI = "/index";
...
//            response.setStatus(HttpServletResponse.SC_NOT_FOUND);
            template = freemarkerConfiguration.getTemplate(URLDecoder.decode("notfound.ftlh"));
        }
    ...
    private Map<String, Object> getData(HttpServletRequest request) {
        ...
        for (Map.Entry<String, String[]> e : request.getParameterMap().entrySet()) {
            if (e.getValue() != null && e.getValue().length == 1 && e.getKey().endsWith("_id")) {
                try {
