...
export default {
    ...
    beforeCreate() {
        ...
        this.$root.$on("onRegister", (login, name, password) => {
            ...
            if (name === "") {
                this.$root.$emit("onRegisterValidationError", "Name is required and have to be shorter than 33 chars");
                return;
            }
            ...
            const users = Object.values(this.users).filter(u => u.login === login);
            if (users.length !== 0) {
                this.$root.$emit("onRegisterValidationError", "Such user already exists");
            } else {
        ...
        this.$root.$on("onComment", (postId, text) => {
            const id = Math.max(...Object.keys(this.comments)) + 1;

            if (this.userId == null) {
                this.$root.$emit("onCommentingError", "U ar not logged in");
                return;
            }
